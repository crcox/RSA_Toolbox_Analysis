...
userOptions = setIfUnset(userOptions, 'saverdms', false);
...
%		subjectMetadataStruct = spawnSPMStruct;
		...
		for maskNumber = 1:nMasks % For each mask...
            ...
            [~,maskbase,~] = fileparts(maskName);
			mask = binaryMasks_nS.(subject).(maskbase);
            ...
            if userOptions.saverdms || nargout >= 3
                ...
                [rs, ps, ns] = searchlightMapping_fMRI(singleSubjectVols, models, mask, userOptions, searchlightOptions); % ps are from linear correlation p-values, and so aren't too useful here.
            end
			
			
            [~,maskbase,~] = fileparts(maskName);
			nMaps_nS.(subject).(maskbase) = ns(:,:,:); % How many voxels contributed to the searchlight centred at each point. (Those with n==1 are excluded because the results aren't multivariate.)
			
			for modelNumber = 1:numel(models)
				% CRC EDIT: spacesToUnderscores does not exist, so I'm
				% hacking this in ...
% 				modelName = spacesToUnderscores(models(modelNumber).name);
                modelName = strrep(models(modelNumber).name, ' ', '_');
				
				% Store results in indexed volumes
				rMaps_nS.(modelName).(subject).(maskbase) = rs(:,:,:,modelNumber); % r-values for correlation with each model
				
				%% Save native space version
				
				% Write the native-space r-map to a file
				rMapMetadataStruct_nS = subjectMetadataStruct;
				rMapMetadataStruct_nS.fname = fullfile(userOptions.rootPath, 'Maps', [userOptions.analysisName '_rMap_' maskbase '_' modelName '_' subject '.img']);
				rMapMetadataStruct_nS.descrip =  'R-map';
				rMapMetadataStruct_nS.dim = size(rMaps_nS.(modelName).(subject).(maskbase));
				
				gotoDir(userOptions.rootPath, 'Maps');
				
				spm_write_vol(rMapMetadataStruct_nS, rMaps_nS.(modelName).(subject).(maskbase));
				
				if isfield(userOptions, 'structuralsPath')

					% Write the native-space mask to a file
					maskMetadataStruct_nS = subjectMetadataStruct;
					maskMetadataStruct_nS.fname = fullfile(userOptions.rootPath, 'Maps', [userOptions.analysisName '_nativeSpaceMask_' maskbase '_' modelName '_' subject '.img']);
...
% 					wildFiles = replaceWildcards(fullfile(userOptions.structuralsPath, ['*' subject '*_seg_sn.mat']), '[[subjectName]]', subject);
					...
					rMaps_sS.(modelName).(subject).(maskbase) = spm_read_vols(spm_vol(fullfile(warpedPath_rMap, [warpedFile_rMap warpedExt_rMap]))); % sS for standard space
					...
					maskMetadataStruct_sS.fname = fullfile(userOptions.rootPath, 'Maps', [userOptions.analysisName '_commonSpaceMask_' maskbase '_' modelName '_' subject '.img']);
					...
					maskedSmoothedRMaps_sS.(modelName).(subject).(maskbase) = maskedData;

					% Write it back to disk
					maskedDataMetadataStruct_nS = smoothedDataMetadataStruct;
					maskedDataMetadataStruct_nS.fname = fullfile(userOptions.rootPath, 'Maps', ['msw' userOptions.analysisName '_rMap_' maskbase '_' modelName '_' subject '.img']); % 'msw' for 'masked, smoothed, warped'
	...
	fprintf('Saving searchlight maps to %s\n', fullfile(userOptions.rootPath, 'Maps', mapsFilename));
	gotoDir(userOptions.rootPath, 'Maps');
	if isfield(userOptions, 'structuralsPath')
    ...
    if userOptions.saverdms
        fprintf('Saving RDMs to %s\n', fullfile(userOptions.rootPath, 'RDMs', RDMsFilename));
        gotoDir(userOptions.rootPath, 'RDMs');
        save(RDMsFilename, 'searchlightRDMs');
    end
	
	fprintf('Saving Details to %s\n', fullfile(userOptions.rootPath, 'Details', DetailsFilename));
